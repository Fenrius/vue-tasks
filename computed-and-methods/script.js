Vue.createApp({
  ...
  computed: {
    sortedHeadNames() {
      ...
      const columnHeads = Object.keys(this.books[0]).filter((bookKey) => {
        return validKeys.includes(bookKey);
      });
      return columnHeads;
    },
    onlyReilly() {
  ...
  methods: {
    discountPrices(money, discount) {
      const [, getPrice] = money.split("$");

      return "$" + (getPrice - (getPrice / 100) * discount).toFixed(2);
    },
  },
}).mount("#app");
